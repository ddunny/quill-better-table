...
export default class TableColumnTool {
  ...
  initColTool() {
    ...
    css(this.domNode, {
      width: `${containerRect.width}px`, // 툴바의 width... 이걸로 변경해도 괜찮은지는 모르겠음;ㅋ
  ...
  updateToolCells() {
    ...
    for (let index = 0; index < Math.max(cellsNumber, existCells.length); index++) {
      let col = tableCols.find(index)[0]; // at은 parchment 2.0.0에 있고 1.1.4에 없음
  ...
  addColCellHolderHandler(cell) {
    ...
    const handleMouseup = (e) => { // 드래그를 마치고 마우스를 뗐을 때
      e.preventDefault();
      ...
      const colBlot = tableContainer.colGroup().children.find(colIndex)[0];
